...
const LoginPage = () => {
  ...
  // Match next.config.ts logic: basePath only in production
  ...
  const handleContinue = () => {
    ...
    if (saveProfile) {
      ...
      try {
        sessionStorage.setItem('agentlee_user', JSON.stringify({ name, email, alias, savedAt: Date.now() }));
      } catch (_) {}
    }
  };
  ...
  return (
    <>
      <LoadingOverlay isLoading={isLoading} onFinished={() => window.location.reload()} />
      <PolicyModal
      ...
      <div className="login-body">
        <main className="card" role="main" aria-labelledby="app-title">
          <div className="logo">
            <Image src={`${basePath}/image/image/AgentLeeavatar.jpeg`} alt="Agent Lee Avatar" width={160} height={160} className="rounded-xl" data-ai-hint="futuristic avatar" />
            <div>
          ...
          <div className="footer-bar" role="contentinfo" style={{ gridArea: "footer" }}>
            ...
            <div className="footer-right">
              <a href="https://rapidwebdevelop.com" target="_blank" rel="noopener" aria-label="RapidWebDevelop">
                <Image src={`/image/image/logo.jpg`} alt="RapidWebDevelop Logo" width={120} height={120} data-ai-hint="corporate logo" />
              </a>
            </div>
          </div>
        </main>
