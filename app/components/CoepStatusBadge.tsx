...
export default function CoepStatusBadge() {
  ...
  useEffect(() => {
    try {
      ...
      let ok = false;
      try { /* @ts-ignore */ ok = !!new SharedArrayBuffer(8); } catch {}
      setSab(ok);
    } catch { setIsolated(false); setSab(false); }
  }, []);
  const pill = (label: string, on?: boolean | null) => (
    <span className={`px-2 py-0.5 rounded-full text-xs border
      ${on===null?'bg-gray-200 text-gray-700 border-gray-300':on?'bg-green-100 text-green-700 border-green-300':'bg-red-100 text-red-700 border-red-300'}`}>
